    ...
    switch (method) {
      case "GET":
        if (req.query.action === "get-relic-sets") {
            const [rows] = await pool.query("SELECT id, body FROM Relic_Set");
            res.status(200).json(rows);
        } else {
            const [rows] = await pool.query("SELECT * FROM Relic_Bodypiece");
            res.status(200).json(rows);
        }
        break;
      ...
      case "POST":
        ...
        await pool.query("INSERT INTO Relic_Bodypiece VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [
        ...
        res.status(201).json({ message: "Relic Bodypiece added successfully" });
        break;

      case "PUT":
        const { id: updateId } = req.body;
        await pool.query(
          "UPDATE Relic_Bodypiece SET main_stat=?, main_stat_num=?, substat_1=?, substat_1_num=?, substat_2=?, substat_2_num=?, substat_3=?, substat_3_num=?, substat_4=?, substat_4_num=?, Relic_Set_id=? WHERE id=?",
        ...
        res.status(200).json({ message: "Relic Bodypiece updated successfully" });
        break;

      case "DELETE":
        const { id: deleteId } = req.query;
        await pool.query("DELETE FROM Relic_Bodypiece WHERE id=?", [deleteId]);
        res.status(200).json({ message: "Relic Bodypiece deleted successfully" });
        break;
