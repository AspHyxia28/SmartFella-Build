...
export default function RelicHandpieceCRUD() {
  const [relicHandpieces, setRelicHandpiece] = useState<RelicHandpiece[]>([]);
  const [relicSets, setRelicSets] = useState<RelicSet[]>([]);
  const [form, setForm] = useState<RelicHandpiece>({
    id: "",
    main_stat: "ATK",
    main_stat_num: 0,
    substat_1: "",
  ...
  useEffect(() => {
    fetchRelicHandpieces();
    fetchRelicSets();
  }, []);

  const fetchRelicHandpieces = async () => {
    const res = await fetch("/api/relic_handpiece");
    const data: RelicHandpiece[] = await res.json();
    setRelicHandpiece(data);
  };

  const fetchRelicSets = async () => {
    const res = await fetch("/api/relic_handpiece?action=get-relic-sets");
  ...
  const handleEdit = (relicHandpiece: RelicHandpiece) => {
    setForm(relicHandpiece);
    setIsEditing(true);
  };

  const handleDelete = async (id: string) => {
    await fetch(`/api/relic_handpiece?id=${id}`, { method: "DELETE" });
    fetchRelicHandpieces();
  };

  return (
    <div className="min-h-screen p-8 bg-no-repeat bg-cover bg-home">
      <h1 className="text-2xl font-bold mb-4">Manage Relic Handpieces</h1> <Navbar />

      <form onSubmit={handleSubmit} className="mb-8 p-4 rounded bg-blue-900">
        <h2 className="text-lg font-bold mb-4">
        {isEditing ? "Edit Relic Handpiece" : "Add New Relic Handpiece"}
        </h2>
        ...
        >
            {isEditing ? "Update Relic Handpiece" : "Add Relic Handpiece"}
        </button>
        </div>
    </form>
      ...
      <table className="w-full bg-blue-900">
        ...
        <tbody>
          {relicHandpieces.map((relicHandpiece) => (
            <tr key={relicHandpiece.id}>
              <td className="border border-gray-300 p-2">{relicHandpiece.id}</td>
              <td className="border border-gray-300 p-2">
                {relicHandpiece.main_stat}
              </td>
              <td className="border border-gray-300 p-2">
                {relicHandpiece.main_stat_num}
              </td>
              <td className="border border-gray-300 p-2">
                {relicHandpiece.substat_1}
              </td>
              <td className="border border-gray-300 p-2">
                {relicHandpiece.substat_1_num}
              </td>
              <td className="border border-gray-300 p-2">
                {relicHandpiece.substat_2}
              </td>
              <td className="border border-gray-300 p-2">
                {relicHandpiece.substat_2_num}
              </td>
              <td className="border border-gray-300 p-2">
                {relicHandpiece.substat_3}
              </td>
              <td className="border border-gray-300 p-2">
                {relicHandpiece.substat_3_num}
              </td>
              <td className="border border-gray-300 p-2">
                {relicHandpiece.substat_4}
              </td>
              <td className="border border-gray-300 p-2">
                {relicHandpiece.substat_4_num}
              </td>
              <td className="border border-gray-300 p-2">
                {relicHandpiece.Relic_Set_id}
              </td>
              <td className="border border-gray-300 p-2">
                <button
                  onClick={() => handleEdit(relicHandpiece)}
                  className="mr-2 p-1 bg-yellow-400 text-white rounded"
                >
                  Edit
                </button>
                <button
                  onClick={() => handleDelete(relicHandpiece.id)}
