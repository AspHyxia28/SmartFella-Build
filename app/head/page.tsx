...
export default function RelicHeadpieceCRUD() {
  const [relicHeadpieces, setRelicHeadpieces] = useState<RelicHeadpiece[]>([]);
  const [relicSets, setRelicSets] = useState<RelicSet[]>([]);
  const [form, setForm] = useState<RelicHeadpiece>({
    id: "",
    main_stat: "HP",
    main_stat_num: 0,
    substat_1: "",
  ...
  useEffect(() => {
    fetchRelicHeadpieces();
    fetchRelicSets();
  }, []);

  const fetchRelicHeadpieces = async () => {
    const res = await fetch("/api/relic_headpiece");
    const data: RelicHeadpiece[] = await res.json();
    setRelicHeadpieces(data);
  };

  const fetchRelicSets = async () => {
    const res = await fetch("/api/relic_headpiece?action=get-relic-sets");
  ...
  const handleEdit = (relicHeadpiece: RelicHeadpiece) => {
    setForm(relicHeadpiece);
    setIsEditing(true);
  };

  const handleDelete = async (id: string) => {
    await fetch(`/api/relic_headpiece?id=${id}`, { method: "DELETE" });
    fetchRelicHeadpieces();
  };

  return (
    <div className="min-h-screen p-8 bg-no-repeat bg-cover bg-home">
      <h1 className="text-2xl font-bold mb-4">Manage Relic Headpieces</h1> <Navbar />

      <form onSubmit={handleSubmit} className="mb-8 p-4 rounded bg-blue-900">
        <h2 className="text-lg font-bold mb-4">
        {isEditing ? "Edit Relic Headpiece" : "Add New Relic Headpiece"}
        </h2>
        ...
        >
            {isEditing ? "Update Relic Headpiece" : "Add Relic Headpiece"}
        </button>
        </div>
    </form>
      ...
      <table className="w-full bg-blue-900">
        ...
        <tbody>
          {relicHeadpieces.map((relicHeadpiece) => (
            <tr key={relicHeadpiece.id}>
              <td className="border border-gray-300 p-2">{relicHeadpiece.id}</td>
              <td className="border border-gray-300 p-2">
                {relicHeadpiece.main_stat}
              </td>
              <td className="border border-gray-300 p-2">
                {relicHeadpiece.main_stat_num}
              </td>
              <td className="border border-gray-300 p-2">
                {relicHeadpiece.substat_1}
              </td>
              <td className="border border-gray-300 p-2">
                {relicHeadpiece.substat_1_num}
              </td>
              <td className="border border-gray-300 p-2">
                {relicHeadpiece.substat_2}
              </td>
              <td className="border border-gray-300 p-2">
                {relicHeadpiece.substat_2_num}
              </td>
              <td className="border border-gray-300 p-2">
                {relicHeadpiece.substat_3}
              </td>
              <td className="border border-gray-300 p-2">
                {relicHeadpiece.substat_3_num}
              </td>
              <td className="border border-gray-300 p-2">
                {relicHeadpiece.substat_4}
              </td>
              <td className="border border-gray-300 p-2">
                {relicHeadpiece.substat_4_num}
              </td>
              <td className="border border-gray-300 p-2">
                {relicHeadpiece.Relic_Set_id}
              </td>
              <td className="border border-gray-300 p-2">
                <button
                  onClick={() => handleEdit(relicHeadpiece)}
                  className="mr-2 p-1 bg-yellow-400 text-white rounded"
                >
                  Edit
                </button>
                <button
                  onClick={() => handleDelete(relicHeadpiece.id)}
